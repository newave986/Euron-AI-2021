...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        # Initialize all self.params dictionary
        ...
        # # Weights should beinitialized from a normal distribution centered at 0 with standard deviation equal to weight_scale
        ...
        # about gamma, beta -> we should run batch normalization
        # self.params['gamma1'] = np.ones(hidden_dim)
        # # Scale(gamma) should be initialized to ones
        # self.params['beta1'] = np.zeros(hidden_dim)
        # # Shift(beta) should be initialized to zeros
        
        layer_input_dim = input_dim 
        
        for i, hd in enumerate(hidden_dims):
            self.params['W%d'%(i+1)] = weight_scale * np.random.rand(layer_input_dim, hd)
            self.params['b%d'%(i+1)] = weight_scale * np.zeros(hd)
            if self.use_batchnorm:
                self.parmas['gamma%d'%(i+1)] = np.ones(hd)
                # initialization
