...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    cache = (x, x_after, gamma, inv_var, sample_mean)
...
def layernorm_backward(dout, cache):
    ...
    x, x_after, gamma, inv_var, sample_mean = cache
    N, D = dout.shape[0], dout.shape[1]
    dgamma = np.ones(N).dot(dout * x_after)
    dbeta = np.ones(N).dot(dout)
    ########## dx ###########
    dx1 = dout * gamma * inv_var
    dx2 = np.ones(N).dot(dout * gamma * (x - sample_mean))
    dx2 *= -(inv_var ** 2)
    dx2 *= 0.5 * inv_var
    dx2 = (1.0 / N) * np.ones((N, D)) * dx2
    dx2 *= 2 * (x - sample_mean)
    dx = dx1 + dx2
    dx1 = dx
    dx2 = -np.ones(N).dot(dx)
    dx2 = (1.0 / N) * np.ones((N, D)) * dx2
    dx = dx1 + dx2
