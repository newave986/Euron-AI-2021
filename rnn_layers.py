...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # at RNN, ht = tanh(Whh*h_(t-1) + Wxh*x_t)
    # h_(t-1)은 prev_h, h_(t)는 next_h이다.
    # tahn 내부를 먼저 inside로 계산해 준 다음,
    # inside를 tahn으로 묶어 next_h를 계산해 준다.
    # backprop에는 x, prev_h, Wh, Wx, b, next_h가
    # 쓰이므로 모두 cache에 저장하여 return한다.
    
    inside = prev_h.dot(Wh) + x.dot(Wx)
...
def rnn_step_backward(dnext_h, cache):
    ...
    dinside = dnext_h * (1-dnext_h * dnext_h)
    # 미분에 의해 d(inside)/dx = Wx, 양변에 dx*(Wx.T)를 취하면 dx 구할 수 있다.
    dx = dinside.dot(Wx.T)
    # 미분에 의해 d(indide)/d(prev_h) = Wh, 양변에 d(prev_h)*(Wh.T)를 취하면 dprev_h 구할 수 있다.
    dprev_h = dinside.dot(Wh.T)
    # 같은 방법으로 편미분을 이용하여 구한다.
    dWx = x.T.dot(dinside)
    dWh = prev_h.T.dot(dinside)
    db = np.sum(dinside, axis = 0)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0 # h0부터 순차적으로 시작
    for t in range(T): #마지막 ht를 구하기까지 T번 수행
        xt = x[:, t, :]
