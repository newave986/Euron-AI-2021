...
Original file is located at
    https://colab.research.google.com/drive/1f_Q96Oy1LPRroipSUwqKfVU6qxZSPKHO
"""
...
## Answer:
[FILL THIS IN]
Initialization scale은 배치 정규화를 사용하는 모델에게는 좀 더 덜한 영향을, 사용하지 않는 모델에게는 더 많은 영향을 끼친다. 왜냐하면 b.n을 통해서는 활성화 값 분포를 적절하게 분배하여 학습이 더 원활하게 되도록 돕기 때문이다. 만약 이것이 없다면 임의로 적절하게 분배하는 과정이 없어 초기값에 훨씬 더 많이 의지해야 할 것이다.
...
## Answer:
[FILL THIS IN]
batch size는 batch normalization을 수행할 때 얼마큼의 사이즈의 배치로 정규화를 수행할 것이냐의 문제이다. 

batch size를 더 늘리면 표본 크기와 더 가까워지므로 정확해지지만 느려질 것이다. 어쨌든간 b.n의 성능을 더욱 좋게 만든다.

https://datascience.stackexchange.com/questions/41873/batch-normalization-vs-batch-size 스택오버플로우의 이 답변 참고했다.
...
## Answer:
[FILL THIS IN]
1번! 각 행에 있는 것대해 합계가 최대 1이 되도록 조정한다 ㅎㅎ
2번은 모든 픽셀에 대해서라고 했으므로 아니다. ㅠ

글구 3번. 왜냐하면 b.n도 shift 작업을 수행하기 때문이다

4번은 one hot encoding 쪽에 더 가깝지 않을까.
...
## Answer:
[FILL THIS IN]
2번. Layer Normalization은 Bath Normalization이므로 feature dim이 작아지면 -> batch size 작아짐. 따라서 normalization이 제대로 작동하지 않을 수 있음.
